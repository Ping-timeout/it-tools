...
-----END CERTIFICATE-----`],HB=gB,rA=K,JB=mB,pB=()=>HB.rootCertificates||JB,aA=D=>!!pB().find(g=>{try{const C=rA.pki.certificateFromPem(g);return rA.pki.certificateToPem(D)===g||C.issued(D)}catch{return!1}}),dA=D=>!!D.find((g,C)=>D[C+1]&&D[C+1].issued(g)),WB=D=>!!D.find(({validity:{notAfter:g,notBefore:C}})=>g.getTime()<Date.now()||C.getTime()>Date.now()),SB=D=>dA(D)&&aA(D[D.length-1]);var kB={authenticateSignature:SB,verifyCaBundle:dA,verifyRootCert:aA,isCertsExpired:WB};const XB=aB,vB=zB,fB=hA,LB=kB;var xA={extractSignature:XB,...vB,...fB,...LB};const KB=K,{extractSignature:OB,getMessageFromSignature:qB,preparePDF:PB}=xA,cA=D=>D.reduce((g,{name:C,value:N})=>(C&&(g[C]=N),g),{}),_B=D=>{const{issuer:g,subject:C,validity:N}=D;return{issuedBy:cA(g.attributes),issuedTo:cA(C.attributes),validityPeriod:N,pemCertificate:KB.pki.certificateToPem(D)}},sA=D=>D.map(_B).map((g,C)=>C?g:{clientCertificate:!0,...g}),$B=D=>{const g=PB(D),{signatureStr:C}=OB(g);return C.map(N=>{const{certificates:T}=qB(N);return sA(T)})};var bA={extractCertificatesDetails:sA,getCertificatesInfoFromPDF:$B};const h=K,iA=BA,{extractSignature:An,getMessageFromSignature:Bn,getClientCertificate:nn,checkForSubFilter:En,preparePDF:Mn,authenticateSignature:Qn,sortCertificateChain:In,isCertsExpired:wn}=xA,{extractCertificatesDetails:Dn}=bA,gn=(D,g,C)=>{const N=Bn(D),{certificates:T,rawCapture:{signature:r,authenticatedAttributes:G,digestAlgorithm:Q}}=N,e=h.asn1.derToOid(Q),o=h.pki.oids[e].toLowerCase(),s=h.asn1.create(h.asn1.Class.UNIVERSAL,h.asn1.Type.SET,!0,G),t=nn(T),z=h.md[o].create().update(h.asn1.toDer(s).data).digest().getBytes();if(!t.publicKey.verify(z,r))throw new iA("Wrong authenticated attributes",iA.VERIFY_SIGNATURE);const k=h.pki.oids.messageDigest,m=G.find(O=>h.asn1.derToOid(O.value[0].value)===k).value[1].value[0].value,H=h.md[o].create().update(g.toString("latin1")).digest().getBytes()===m,J=In(T),a=Dn(J),p=Qn(J),W=wn(J);return{verified:H&&p&&!W,authenticity:p,integrity:H,expired:W,meta:{certs:a,signatureMeta:C}}};var Cn=D=>{const g=Mn(D);En(g);try{const{signatureStr:C,signedData:N,signatureMeta:T}=An(g),r=N.map((G,Q)=>gn(C[Q],G,T[Q]));return{verified:r.every(G=>G.verified===!0),authenticity:r.every(G=>G.authenticity===!0),integrity:r.every(G=>G.integrity===!0),expired:r.some(G=>G.expired===!0),signatures:r}}catch(C){return{verified:!1,message:C.message,error:C}}};const uA=Cn,{getCertificatesInfoFromPDF:Nn}=bA;Object.assign(uA,{getCertificatesInfoFromPDF:Nn});var Gn=uA;const Vn=_A(Gn),Tn={style:{flex:"0 0 100%"}},tn={"mx-auto":"","max-w-600px":""},Fn={"font-bold":""},Un={key:1},rn={key:0,style:{flex:"0 0 100%"},"mt-5":"",flex:"","flex-col":"","gap-4":""},cn={"mb-2":"","font-bold":""},fn=YA({__name:"pdf-signature-checker",setup(D){const{t:g}=lA(),C=_([]),N=_("idle"),T=_(null);async function r(G){T.value=G;const Q=await G.arrayBuffer();N.value="loading";try{const{signatures:e}=Vn(Q);C.value=e,N.value="parsed"}catch{C.value=[],N.value="error"}}return(G,Q)=>{const e=nB,o=EB,s=MB,t=GB;return u(),j(TA,null,[S("div",Tn,[S("div",tn,[x(e,{title:U(g)("tools.pdf-signature-checker.texts.title-drag-and-drop-a-pdf-file-here-or-click-to-select-a-file"),accept:".pdf",onFileUpload:r},null,8,["title"]),U(T)?(u(),$A(o,{key:0,"mt-4":"",flex:"","gap-2":""},{default:Z(()=>[S("div",Fn,f(U(T).name),1),S("div",null,f(U(CB)(U(T).size)),1)]),_:1})):$("",!0),U(N)==="error"?(u(),j("div",Un,[x(s,{"mt-4":""},{default:Z(()=>[AB(f(U(g)("tools.pdf-signature-checker.texts.tag-no-signatures-found-in-the-provided-file")),1)]),_:1})])):$("",!0)])]),U(N)==="parsed"&&U(C).length?(u(),j("div",rn,[(u(!0),j(TA,null,BB(U(C),(z,b)=>(u(),j("div",{key:b},[S("div",cn," Signature "+f(b+1)+" certificates : ",1),x(t,{signature:z},null,8,["signature"])]))),128))])):$("",!0)],64)}}});export{fn as default};
